...
public class _OkcoinInternational  implements ExchangeAPIInterface {

    public ReturnData_Ticker getTickerReturnData(String symbol) {
        final String FetchURL;
        // this_week   next_week   month   quarter
        // btc Futures Quarterly_usd
        // https://www.okcoin.com/api/v1/future_ticker.do?symbol=btc_usd&contract_type=this_week

        if (symbol.contains("Futures"))
        {
            String[] split1 = symbol.split("_");
            String[] split2 = split1[0].split(" ");

            FetchURL = String.format("https://www.okcoin.com/api/v1/future_ticker.do?symbol=%s&contract_type=%s",
            ...
            FetchURL = String.format("https://www.okcoin.com/api/ticker.do?symbol=%s&ok=1", symbol);
    ...
    public ReturnData_Depth getDepthReturnData(String symbol, double currentBuyPrice, double currentSellPrice) {
        ...
        // this_week   next_week   month   quarter
        // btc Futures Quarterly_usd
        // https://www.okcoin.com/api/v1/future_depth.do?symbol=btc_usd&contract_type=this_week
        if (symbol.contains("Futures"))
        {
            String[] split1 = symbol.split("_");
            String[] split2 = split1[0].split(" ");

            FetchURL = String.format("https://www.okcoin.com/api/v1/future_depth.do?symbol=%s&contract_type=%s",
                    String.format("%s_%s", split2[0], split1[1]),
                    split2[2] == "Quarterly" ? "quarter" : (split2[2] == "Monthly" ? "month" : (split2[2] == "BiWeekly" ? "next_week" : "this_week"))
            );
        }
        else
            FetchURL = String.format("https://www.okcoin.com/api/depth.do?symbol=%s&ok=1", symbol);
    ...
    public ReturnData_Trades getTradesReturnData(String symbol, String pairNameLeft, String pairNameRight, long since, int firstTradeId) {
        ...
        if (symbol.contains("Futures"))
        {
            ...
            FetchURL = String.format("https://www.okcoin.com/api/v1/future_trades.do?symbol=%s&contract_type=%s",
                    String.format("%s_%s", split2[0], split1[1]),
                    split2[2] == "Quarterly" ? "quarter" : (split2[2] == "Monthly" ? "month" : (split2[2] == "BiWeekly" ? "next_week" : "this_week"))
            );
        }
        else
            FetchURL = String.format("https://www.okcoin.com/api/trades.do?symbol=%s&ok=1", symbol);
Log.i("bitbot", FetchURL);
        final String ret = HttpClientWrapper.queryHttpHttps(FetchURL);
        if (ret == null)
            return null;
