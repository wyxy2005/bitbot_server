exports.post = function(request, response) {
        ...
        var exchange_pair = exchangesite + '_' + currencypair;
              ...
              pushTable.where({
                    ...
                    success: function(results) {
                        if (results.length != 0) {
                            // a copy available
                            updateDatabase(tableName, high, low, times, pushuri, delay_between_notification, uniqueid, increment_percent_daily, exchange_pair,platform);
                        } else {
                            // insert
                            pushTable.insert(
                                { 
    ...
    function updateDatabase(tableName, high, low, times, pushuri, delay_between_notification, uniqueid, increment_percent_daily, exchange_pair, platform) {
        var mssql = request.service.mssql;
        var query = "UPDATE TOP (1) push_price SET pushuri = ?, high = ?, low = ?, times = ?, delay_between_notification = ?, increment_percent_daily = ? WHERE uniqueid = ? AND exchange_pair = ? AND operating_system = ?";
         
        // console.log(query)
        
          mssql.query(query, [pushuri, high, low, times, delay_between_notification, increment_percent_daily, uniqueid, exchange_pair, platform], {
               success: function(results) {            
                }, error:function(err) {
