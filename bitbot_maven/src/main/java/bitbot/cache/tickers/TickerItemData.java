...
public class TickerItemData implements TickerItem {
    ...
    /*public void mergeUnmaturedData(long server_time, float close, float high, float low, float open, double volume, double volume_cur, float buysell_ratio, boolean unmatured_data) {
        if (unmatured_data) {
            this.server_time = server_time;//DateTimeUtil.convertDateTime(Long.parseLong(obj.get("server_time").toString()));
        }
        this.high = high;
