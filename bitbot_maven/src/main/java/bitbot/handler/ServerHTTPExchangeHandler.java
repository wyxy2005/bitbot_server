...
public class ServerHTTPExchangeHandler implements Container {
    ...
    public void handle(Request request, Response response) {
        ...
        if (ChannelServer.getInstance().isEnforceCloudFlareNetwork()) {
            if (CloudFlareForwardAddress == null
                    || // No forward address yet we are enforcing "cloud-flare" only,
        ...
        processHttpRequest(request, response, IPAddress);
    }

    /**
     * Process the HTTP request called by the client.
     * 
     * @param request The request object by the client
     * @param response The response object for the server
     * @param IPAddress The IP address of the client. This may be a forward address by CloudFlare
     */
    private static void processHttpRequest(Request request, Response response, String IPAddress) {
        // Query
